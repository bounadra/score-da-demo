...
RUNS_PATH = Path(os.environ.get('SDA_RUNS_PATH', PATH / 'runs_post1'))
...
def evaluation(i: int, name: str, local: bool, freq: str):
    ...
    with h5py.File(RESULTS_PATH / 'obs.h5', mode='r') as f:
        y = torch.from_numpy(f[freq][i]).to(DEVICE)
    ...
    # Ground truth
    x = posterior(y, A=A, sigma=sigma, step=step, device=DEVICE)[:1024]
    x_ = posterior(y, A=A, sigma=sigma, step=step, device=DEVICE)[:1024]

    log_px = log_prior(x).mean().item()
    log_py = log_likelihood(y, x, A=A, sigma=sigma, step=step).mean().item()
    # POT's EMD is CPU-oriented; keep only this metric on CPU.
    w1 = emd(x.cpu(), x_.cpu()).item()
    ...
    # Score
    score = load_score(RUNS_PATH / f'{name}/state.pth', local=local, device=str(DEVICE))
    sde = VPSDE(
        GaussianScore(
            y=y,
            A=lambda x: x[..., ::step, :1],
    ...
    for C in (0, 1, 2, 4, 8, 16):
        x = sde.sample((1024,), steps=256, corrections=C, tau=0.25)
